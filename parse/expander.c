		...
		// 25'den fazla satır uzunluğı
{
	...
	while (tmp)
	{
		...
		{
			if (tmp->word[i] == '\'')
				// tek tırnak varsa direk geçicek atlanıcak işlem yok.
			{
				i++;
				...
				// bunun dışında her kelimede $ varsa expander etcek
			{
				if (tmp->word[i + 1] == 0)
					return ;
				...
				while (tmp->word[i] && (ft_isalnum(tmp->word[i])
						|| tmp->word[i] == '_'))
					// değişkenin sonuna kadar gidilir.
					i++;
				control_expender(start, i, env, tmp);
				if (i - 1 != start)
					i = 0;
						// her bir kelime için sıfır yapılır ki baştan tekrar döngüye girsin diye.
			}
			else
				i++;
		}
